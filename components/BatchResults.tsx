...
import { CheckCircle2, XCircle, Loader2, AlertCircle, Building, MapPin, Hash, Utensils } from 'lucide-react';
...
const BatchResults: React.FC<BatchResultsProps> = ({ items, mode }) => {
  return (
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <th className="px-6 py-4 font-semibold text-slate-700">商家信息</th>
              {mode === 'ADDRESS' ? (
                   <th className="px-6 py-4 font-semibold text-slate-700">地址比对</th>
              ) : (
                   <th className="px-6 py-4 font-semibold text-slate-700">菜品比对</th>
              )}
              <th className="px-6 py-4 font-semibold text-slate-700 text-center w-32">结果</th>
              <th className="px-6 py-4 font-semibold text-slate-700 w-64">AI 分析</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {items.map((item, index) => {
                const addressResult = mode === 'ADDRESS' ? (item.result as AddressAnalysisResult) : undefined;
                const dishResult = mode === 'DISH' ? (item.result as DishAnalysisResult) : undefined;
                    ...
                    {mode === 'ADDRESS' ? (
                        <td className="px-6 py-4 space-y-2">
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              <span className="text-xs text-slate-400 block">实际地址:</span>
                              <span className="text-slate-700">{item.realAddress}</span>
                              {addressResult && (
                                <span className="ml-2 text-xs bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded">
                                  {addressResult.realAddressDistrict}
                                </span>
                              )}
                            </div>
                          </div>
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              <span className="text-xs text-slate-400 block">推荐商圈:</span>
                              ...
                              {addressResult && (
                                 <span className="ml-2 text-xs bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded">
                                   {addressResult.recommendedAddressDistrict}
                                 </span>
                              )}
                            </div>
                          </div>
                         ...
                         <td className="px-6 py-4 space-y-2">
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              <span className="text-xs text-slate-400 block">上新菜品:</span>
                              <span className="text-slate-900 font-medium">{item.spuName}</span>
                              <div className="text-[10px] text-slate-400 font-mono mt-0.5">SPU ID: {item.spuId}</div>
                            </div>
                          </div>
                          <div className="flex items-start gap-2">
                            <Utensils className="w-4 h-4 text-indigo-500 mt-0.5 shrink-0" />
                            <div>
                              <span className="text-xs text-slate-400 block">推荐参考:</span>
                              <span className="text-slate-700">{item.recommendDishName}</span>
                            </div>
                          </div>
                        </td>
                    )}
